...
def clean_data(gt_anno, dt_anno, current_class, difficulty):
    ...
    MIN_HEIGHT = [0,0,0]#[40, 25, 25]
    MAX_OCCLUSION = [-1,-1,-1]#[0, 1, 2]
    MAX_TRUNCATION = [-1,-1,-1]#[0.15, 0.3, 0.5]
    ...
    for j, curcls in enumerate(current_classes):
        ...
        for i in range(min_overlaps.shape[0]):
            ...
            if i == 1:
                ...
                mean_AP_list_3d.append((mAP3d[j, 0, 0] + mAP3d[j, 1, 0] + mAP3d[j, 2, 0]) / 3)
                mean_AP_list_bev.append((mAPbev[j, 0, 0] + mAPbev[j, 1, 0] + mAPbev[j, 2, 0]) / 3)

    ret_dict['mAP_3d'] = sum(mean_AP_list_3d) / len(mean_AP_list_3d)
    ret_dict['mAP_bev'] = sum(mean_AP_list_bev) / len(mean_AP_list_bev)
