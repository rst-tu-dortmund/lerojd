...
def transform_annotations_to_kitti_format(annos, map_name_to_kitti=None, info_with_fakelidar=False):
    ...
    for anno in annos:
        ...
        anno['id'] = np.zeros(len(anno['name']))
...
def random_downsample(points, seed, sample_rate):
    np.random.seed(seed)
    random_indices = np.random.choice(points.shape[0], size = int(len(points) * sample_rate), replace = False )
    downsampled_points = np.delete(points, random_indices, axis=0)
