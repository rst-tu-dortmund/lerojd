...
This method plots the ground truth labels on the frame.
        ...
        # filtered = list(filter(lambda elem: elem['range'] > min_distance_threshold, filtered))

        colors = [label_color_palette_2d[v["label_class"]] for v in filtered]
        labels = [d['corners'] for d in filtered]


        listofi = [] #= [-1 for i in range(labels.__len__())]
        for i in range(labels.__len__()):
            for j in range(8):
                for k in range(2):
                    if labels[i][j][k] < 0:
                        listofi.append(i) #[i] = i

        listofi = list(dict.fromkeys(listofi))

        for index in sorted(listofi, reverse=True):
            del labels[index]
            del colors[index]
