...
from . import VoD_utils_lidar
from ...ops.roiaware_pool3d import roiaware_pool3d_utils
from ...utils import box_utils, calibration_kitti, common_utils, object3d_kitti
from ..dataset import DatasetTemplate


class VoDDataset_lidar_radar(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        ...
        self.root_path_2 = Path(self.dataset_cfg.DATA_PATH_2)
        ...
        if self.dataset_cfg.get('DOWNSAMPLE',None):
            ...
            if self.sample_rate == 50:
                self.mask_file = os.path.join(self.root_path, self.sample_method, 'R_50')
            elif self.sample_rate == 25:
                self.mask_file = os.path.join(self.root_path, self.sample_method, 'R_25')
            elif self.sample_rate == 12.5:
                self.mask_file = os.path.join(self.root_path, self.sample_method, 'R_12')
            elif self.sample_rate == 6.25:
                self.mask_file = os.path.join(self.root_path, self.sample_method, 'R_06')
            elif self.sample_rate == 3.125:
                self.mask_file = os.path.join(self.root_path, self.sample_method, 'R_03')
            elif self.sample_rate == 1.5625:
                self.mask_file = os.path.join(self.root_path, self.sample_method, 'R_01')
            elif self.sample_rate == 0.78125:
                self.mask_file = os.path.join(self.root_path, self.sample_method, 'R_007')
            elif self.sample_rate == 0.390625:
                self.mask_file = os.path.join(self.root_path, self.sample_method, 'R_004')
    ...
    def include_VoD_data(self, mode):
        ...
        for info_path in self.dataset_cfg.INFO_PATH[mode]:
            info_path = self.root_path_2 / info_path
            if not info_path.exists():
                continue
            with open(info_path, 'rb') as f:
                infos_2 = pickle.load(f)
                VoD_infos_2.extend(infos_2)

        self.VoD_infos_2.extend(VoD_infos_2)
    ...
    def get_lidar(self, idx):
        ...
        data_locations = KittiLocations(root_dir=str(self.root_path)[:-6],output_dir="")
        frame_data = FrameDataLoader(kitti_locations=data_locations,
                                     frame_number=idx)
        transforms = FrameTransformMatrix(frame_data)
        radar_xyz = np.hstack([radar_np_RCS[:, :3], np.ones([len(radar_np_RCS),1])])
        radar_transformed = homogeneous_transformation(radar_xyz, transforms.t_lidar_radar.round(2))

        radar_np_RCS[:, :3] = radar_transformed[:, :3] 

        #### Join radar and lidar ####
        invalid_value = 0  
        padding_lidar = np.hstack([lidar_np_RCS[:,:3], np.zeros([len(lidar_np_RCS),4])+invalid_value, lidar_np_RCS[:,3:]])
        ...
        lidar_radar_np = np.vstack([padding_lidar, padding_radar])
    ...
    def __getitem__(self, index):
        ...
        if "calib_matricies" in get_item_list:
            input_dict["trans_lidar_to_cam"], input_dict["trans_cam_to_img"] = VoD_utils_lidar.calib_to_matricies(calib)
...
def create_VoD_lidar_infos(dataset_cfg, class_names, data_path, save_path, workers=4):
    dataset = VoDDataset_lidar_radar(dataset_cfg=dataset_cfg, class_names=class_names, root_path=data_path, training=False)
...
if __name__ == '__main__':
    import sys
    if sys.argv.__len__() > 1 and sys.argv[1] == 'create_VoD_lidar_infos':
        import yaml
        from pathlib import Path
        from easydict import EasyDict
        dataset_cfg = EasyDict(yaml.safe_load(open(sys.argv[2])))
        ROOT_DIR = (Path(__file__).resolve().parent / '../../../').resolve()
        create_VoD_lidar_infos(
