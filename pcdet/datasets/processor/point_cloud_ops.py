...
@numba.jit(nopython=True)
def _points_to_voxel_lira_reverse_kernel(
    points,
    voxel_size,
    coors_range,
    num_points_per_voxel,
    ...
    # we shouldn't create large array in main jit code, otherwise
    # reduce performance
    points_radar = points[points[:, 7] == 0, :]
    points_lidar = points[points[:, 6] == 0, :]

    N_r = points_radar.shape[0]
    N_l = points_lidar.shape[0]
    # ndim = points.shape[1] - 1
    ndim = 3
    ndim_minus_1 = ndim - 1
    grid_size = (coors_range[3:] - coors_range[:3]) / voxel_size
    ...
    for i in range(N_r):
        failed = False
        for j in range(ndim):
            c = np.floor((points_radar[i, j] - coors_range[j]) / voxel_size[j])
            if c < 0 or c >= grid_size[j]:
                failed = True
                break
            coor[ndim_minus_1 - j] = c
        ...
        num = num_points_per_voxel[voxelidx]
        if num < max_points:
            voxels[voxelidx, num] = points_radar[i]
            num_points_per_voxel[voxelidx] += 1

    for i in range(N_l):
        failed = False
        for j in range(ndim):
            c = np.floor((points_lidar[i, j] - coors_range[j]) / voxel_size[j])
            if c < 0 or c >= grid_size[j]:
                failed = True
                break
            coor[ndim_minus_1 - j] = c
        ...
        num = num_points_per_voxel[voxelidx]
        if num < max_points:
            voxels[voxelidx, num] = points_lidar[i]
            num_points_per_voxel[voxelidx] += 1
    return voxel_num
...
@numba.jit(nopython=True)
def _points_to_voxel_lira_kernel(
    points,
    voxel_size,
    coors_range,
    num_points_per_voxel,
...
def points_to_voxel(
    ...
    if points.shape[1] == 4 or points.shape[1] == 7 or points.shape[1] == 3:
        if reverse_index:
        ...
        if reverse_index:
            voxel_num = _points_to_voxel_lira_reverse_kernel(
                points,
                voxel_size,
                coors_range,
                num_points_per_voxel,
            ...
            voxel_num = _points_to_voxel_lira_kernel(
                points,
                voxel_size,
                coors_range,
                num_points_per_voxel,
