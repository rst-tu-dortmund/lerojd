...
from ..model_utils.BinningHistogramm import match_bbox
import numpy as np
...
class Detector3DTemplate(nn.Module):
    def __init__(self, model_cfg, num_class, dataset):
        ...
        self.matched_cls = np.empty([0,3])
        self.matched_gt = np.empty([0, 3])
    ...
    def post_processing(self, batch_dict):
        ...
        for index in range(batch_size):
            ...
            if post_process_cfg.NMS_CONFIG.MULTI_CLASSES_NMS:
                ...
                # selected, selected_scores = model_nms_utils.class_agnostic_nms_classes(
                #     box_scores=cls_preds, box_preds=box_preds,
                #     nms_config=post_process_cfg.NMS_CONFIG,
                #     label_preds=label_preds,
                #     score_thresh_list=post_process_cfg.SCORE_THRESH
                # )
            ...
            # recall_dict = self.generate_recall_record_each_class(
            #     box_preds_all=final_boxes if 'rois' not in batch_dict else src_box_preds,
            #     recall_dict=recall_dict, batch_index=index, label_preds=final_labels, data_dict=batch_dict,
            #     thresh_list=post_process_cfg.RECALL_THRESH_LIST
            # )
    ...
    @staticmethod
    def generate_recall_record_each_class(box_preds_all, recall_dict, batch_index, label_preds, data_dict=None,
                                          thresh_list=None):
        if 'gt_boxes' not in data_dict:
            return recall_dict

        thresh_list = torch.tensor(thresh_list)
        ...
        if recall_dict.__len__() == 0:
            ...
            for cur_class in classnames:
                recall_dict['rcnn_%s' % cur_class] = 0
                recall_dict['gt_%s' % cur_class] = 0

        cur_gt_all = gt_boxes
        k = cur_gt_all.__len__() - 1
        while k >= 0 and cur_gt_all[k].sum() == 0:
            k -= 1
        cur_gt_all = cur_gt_all[:k + 1]

        for i, cur_class in enumerate(classnames):
            cur_gt = cur_gt_all[cur_gt_all[:, 7] == i + 1]
            box_preds = box_preds_all[label_preds == i + 1, :]
            if cur_gt.shape[0] > 0:
                ...
                if iou3d_rcnn.shape[0] == 0:
                    recall_dict['rcnn_%s' % str(cur_class)] += 0
                else:
                    rcnn_recalled = (iou3d_rcnn.max(dim=0)[0] > thresh_list).sum().item()
                    recall_dict['rcnn_%s' % str(cur_class)] += rcnn_recalled
                if rois is not None:
                    roi_recalled = (iou3d_roi.max(dim=0)[0] > thresh_list).sum().item()
                    # recall_dict['roi_%s' % str(cur_thresh)] += roi_recalled

                recall_dict['gt_%s' % cur_class] += cur_gt.shape[0]
            else:
