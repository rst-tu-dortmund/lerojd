    ...
    def forward(self, batch_dict):
        ...
        Returns:
            bacth_dict (dict):
                ...
                - ...
        '''
        # set_voxel_inds_list_befor = [[batch_dict[f'set_voxel_inds_stage{s}_shift{i}'] for i in range(self.num_shifts[s])] for s in range(self.stage_num)]
        # set_voxel_masks_list_befor = [[batch_dict[f'set_voxel_mask_stage{s}_shift{i}'] for i in range(self.num_shifts[s])] for s in range(self.stage_num)]
        # pos_embed_list_befor = [[[batch_dict[f'pos_embed_stage{s}_block{b}_shift{i}'] for i in range(self.num_shifts[s])] for b in range(self.set_info[s][1])] for s in range(self.stage_num)]
